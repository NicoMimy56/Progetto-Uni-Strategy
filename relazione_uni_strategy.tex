...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Client}: i moduli sotto \texttt{js/} (es.\ \texttt{store.js}, \texttt{constants.js}) tengono lo \texttt{state} (esami, piano studio, simulazioni, profilo, obiettivo media) popolato da \texttt{/api/bootstrap} dopo il login; \texttt{app.js} orchestra rendering ed eventi.
\end{itemize}

\section{Backend: cartella \texttt{server/} e \texttt{server.js}}

Il file \texttt{server.js} in radice avvia HTTP sulla porta configurabile; la logica Express è in \texttt{server/createApp.js} (middleware, statici e fallback SPA), mentre \texttt{server/database.js}, \texttt{server/auth.js}, \texttt{server/mappers.js}, \texttt{server/routes.js}, \texttt{server/paths.js} e \texttt{server/config.js} separano persistenza, autenticazione, mapping JSON e registrazione API.
...
\section{Frontend: moduli JavaScript (\texttt{js/})}

\subsection{File e responsabilità}
\texttt{constants.js} raggruppa costanti (giorni, lingue supportate, preset tema, CFU default).\\
\texttt{store.js} espone oggetti \texttt{state}, \texttt{ui}, \texttt{studyTimePickerState}, \texttt{currentCalendarDate} e funzioni \texttt{getExams}/\texttt{saveExams}/\ldots\\
\texttt{dom.js} esporta riferimenti \texttt{getElementById} agli elementi principali dell'HTML.\\
\texttt{api.js} contiene solo \texttt{apiRequest}.\\
\texttt{academic.js} calcoli puri (\texttt{weightedGpa}, \texttt{simulatedGpa}, \texttt{daysRemaining}).\\
\texttt{app.js} (lungo): tema, i18n, picker date/ora, canvas andamento, calendario, piano studio, \texttt{render()}, listener e bootstrap.
